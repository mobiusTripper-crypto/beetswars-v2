import { OverallStats } from "types/overallstats.trpc";
...
export default async function getOverallStats(): Promise<OverallStats | null> {
  ...
  const bribedVotesRatio = Number(
    ((bribedVotes / totalVotes) * 100).toFixed(2)
  );
  ...
  const votingApr = data.map((round) => {
    const x =
      (round.totalBribes / round.priceFbeets / round.bribedVotes) * 2600;
    return !x ? 0 : x;
  });
  const avgVotingApr =
    (votingApr.reduce((sum, a) => sum + a ?? 0) / rounds).toFixed(2) + " %";
